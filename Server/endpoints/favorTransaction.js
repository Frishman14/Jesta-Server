...
exports.favorTransactionResolvers = {
    ...
    Query: {
        ...
        getAllOwnerFavorTransactionByStatus: async (parent, args, context) => { return isAuthenticated(context) ? await favorTransaction.find({status: JESTA_TRANSACTION_STATUS[args.status], ownerId: context.sub}).populate("favorOwnerId favorId handledByUserId").exec() : new AuthenticationError("unauthorized"); },
